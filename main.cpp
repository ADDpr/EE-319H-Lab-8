...
void PortF_Init(void){
	//activates the clock for Port F
	SYSCTL_RCGCGPIO_R |= 0x20;  
	//it waits until it stablizes
	while((SYSCTL_RCGCGPIO_R & 0x20) != 0x20) {}; 
	//sets PF1,2,3 as output
	GPIO_PORTF_DIR_R |= 0x0E;  
	//Sets PF4 as not output
	GPIO_PORTF_DIR_R &= 0xEF;  
	//pulldown resistor
	GPIO_PORTF_PUR_R |= 0x10;  
	//enable digital I/O pins for Port F
...
// 12-bit ADC
uint32_t Data;   
// 32-bit fixed-point 0.01 cm
uint32_t Position;  
// single step this program and look at Data
int main1(void){      
  DisableInterrupts();
	// start scope set system clock to 80 MHz
  TExaS_Init();  
	// turn on ADC, PD2, set channel to 5
  ADC_Init(SAC_NONE);    
  EnableInterrupts();
  while(1){           
    // sample 12-bit channel 5, PD2		
    Data = ADC_In();  
  }
}
...
int main(void){  
 ...
 // 32-point averaging 
  ...
  while(1){
		...
		int testDistance = my.Distance();
		
		SSD1306_SetCursor(0,0);
		LCD_OutFix(testDistance);
...
int main5(void){ uint32_t time; 
 ...
 // 32-point averaging  
  ...
  EnableInterrupts();
  while(1){
		// approximate 4 Hz sampling rate
    Delay1ms(227);     
		// Heartbeat
    PF3 ^= 0x08;       
    Data = ADC_In();
